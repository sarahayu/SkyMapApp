...
*  SPI.
*/
    // Signal LOW = command, see https://github.com/adafruit/Adafruit-SSD1351-library/blob/a0a56bc6cda293790ff269cbf3a7525b77c30392/Adafruit_SSD1351.cpp#L70
    setDCLow();
    MAP_SPIDataPut(GSPI_BASE,c);
    // without this line, data put hangs because we need to flush the thingie? even tho it's write-only? weird
...
*  SPI.
*/
    // Signal HIGH = data, see https://github.com/adafruit/Adafruit-SSD1351-library/blob/a0a56bc6cda293790ff269cbf3a7525b77c30392/Adafruit_SSD1351.cpp#L70
    setDCHigh();
    MAP_SPIDataPut(GSPI_BASE,c);
    // without this line, data put hangs because we need to flush the thingie? even tho it's write-only? weird
    MAP_SPIDataGet(GSPI_BASE,&ulDummy);
...
//TODO 3 (Sarah: IDK if reset works yet, Pranav double check this?)
...
*/
  ...
  setRESETLow();            // RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  setRESETHigh();    // RESET = RESET_HIGH
